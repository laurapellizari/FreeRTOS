...
# 52 "C:\Program Files\Microchip\xc8\v2.20\pic\include\proc\pic18f4520.h"
...
next = start;
for (p = start % 10; p != end; p = (p + 1) % 10) {
if((pool[p]->start == pool[next]->start)&(pool[p]->priority > pool[next]->priority)){
next = p;
}
else if(pool[p]->start < pool[next]->start){
next = p;
}
}
...
void KernelClock(void) {
unsigned char proc;
for (proc = start; proc != end; proc = (proc + 1) % 10) {
if ((pool[proc]->start)>(-30000)) {
pool[proc]->start--;
}
}
}
